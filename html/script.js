...
function closeMenu() {
    $.post('https://wcuk_mdw/close', JSON.stringify({}));
...
function revokeLicense(t, license) {
    ...
    if (checkPerms('revokelicense')) {
        ...
        $.post('https://wcuk_mdw/revokeLicense', JSON.stringify({
            person: profile,
            license: license
        }));
        $(t).parent().remove();
...
function saveProfile() {
    ...
    if (checkPerms('editprofile') && profile != null) {
        ...
        $.post('https://wcuk_mdw/updateProfile', JSON.stringify({
            person: profile,
            description: desc,
            picture: picture
        }));
...
function setHouseWaypoint(property) {
    ...
    $.post('https://wcuk_mdw/setHouseWaypoint', JSON.stringify({
...
function chooseToAdd(k) {
    ...
    if (addingField == 'evidence') {
        $.post('https://wcuk_mdw/addToIncidentEvidence', JSON.stringify({
            incident: incident,
            field: addingField,
            id: k,
            incidents: incidents
        ...
        $.post('https://wcuk_mdw/addToIncident', JSON.stringify({
            incident: incident,
            field: addingField,
            identifier: k,
...
function createIncident() {
    ...
    if (checkPerms('createincident')) {
        if ($('#u830-4').text() == "NEW INCIDENT") {
            ...
            $.post('https://wcuk_mdw/createIncident', JSON.stringify({
                incident: incident
            }));
...
function removeIncident() {
    ...
    if (incident != null && checkPerms('deleteincident')) {
        ...
        $.post('https://wcuk_mdw/removeIncident', JSON.stringify({
            incident: incident
        }));
        $("#" + incident).remove();
...
function saveStaffProfile() {
    ...
    if (checkPerms('administrator') && staffprofile != null) {
        ...
        $.post('https://wcuk_mdw/saveStaffProfile', JSON.stringify({
            person: staffprofile,
            picture: picture,
            alias: alias,
...
function saveIncident() {
    ...
    if (incident != null && checkPerms('editincident')) {
        ...
        if (title != '') {
            ...
            $.post('https://wcuk_mdw/saveIncident', JSON.stringify({
                title: title,
                location: location,
                time: time,
                incident: incident,
...
function sentanceForCharges() {
   ...
   $.post('https://wcuk_mdw/sentance', JSON.stringify({
            jail: chargeJail,
            fine: chargeFine,
...
function saveEvidence() {
    ...
    if ($('#u2714-4').text() == 'NEW EVIDENCE') {
        ...
        $.post('https://wcuk_mdw/createEvidence', JSON.stringify({
            id: openedEvidence,
            image: image,
        ...
        if (evidence[openedEvidence].type == 'evidence') {
            ...
            $.post('https://wcuk_mdw/saveEvidence', JSON.stringify({
                id: openedEvidence,
                image: image,
                description: description
            }));
...
function removeReport() {
    ...
    if (report != null && checkPerms('deletereport')) {
        ...
        $('#' + report).remove();
        $.post('https://wcuk_mdw/removeReport', JSON.stringify({
            id: report
        }));
        newReport();
...
function removeEvidence() {
    ...
    if (openedEvidence != null) {
        if (evidence[openedEvidence].type == 'evidence') {
            ...
            $.post('https://wcuk_mdw/removeEvidence', JSON.stringify({
                id: openedEvidence
            }));
            $('#evidenceMenu').html('');
...
function saveReport() {
    ...
    if ($('#u1522-4').text() == 'NEW REPORT' && title != "" && report_incident != "" && incidents[report_incident] != null && reportStatus != "") {
        ...
        if (reportBroadcast != "") {
            ...
            $.post('https://wcuk_mdw/broadcast', JSON.stringify({
        ...
        $.post('https://wcuk_mdw/createReport', JSON.stringify({
            title: title,
            id: report,
            description: description,
        ...
        if (title != "" && checkPerms('editreport')) {
            ...
            $.post('https://wcuk_mdw/updateReport', JSON.stringify({
                title: title,
                id: report,
                description: description,
                incident: report_incident,
...
window.addEventListener('message', function(event) {
    ...
    if (edata.type == "open") {
        ...
        if (users[identifier] == null) {
            console.log('[WCUK MDT] Users not fully loaded!');
            return;
        }
